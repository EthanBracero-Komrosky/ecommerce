--initial look
SELECT * FROM "user_events.csv" LIMIT 1000;
...
WITH funnel_stages AS (
    ...
    WHERE event_date >= (SELECT MAX(event_date) from 'user_events.csv') - INTERVAL 30 DAY
)
...
SELECT
    ...
    ROUND(purchases * 100 / carts) AS purchase_to_cart_conversion_rate,
FROM source_funnel
...
SELECT
    ...
    ROUND(AVG(date_diff('minute', view_time, cart_time)::DOUBLE), 2) AS avg_view_to_cart_minutes,
    ROUND(AVG(date_diff('minute', cart_time, purchase_time)::DOUBLE), 2) AS avg_cart_to_purchase_minutes,
    ROUND(AVG(date_diff('minute', view_time, purchase_time)::DOUBLE), 2) AS avg_total_journey_minutes
FROM user_journey;
